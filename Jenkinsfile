pipeline {
    ...
    stages{
        stage('Build maven'){
            steps{
                checkout scmGit(branches: [[name: '*/master']], extensions: [], userRemoteConfigs: [[url: 'https://github.com/vicho314/TINGESO-1-Backend']])
        ...
        stage('Build docker image'){
            steps{
                script{
                    sh 'docker build -t vicho314/kartingrm-backend:latest .'
                }
            }
        }
